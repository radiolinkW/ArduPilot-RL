...
def build(bld):
    ...
    # Check if we should use prebuilt libch.a
    # Priority: Environment variable > File configuration
    env_use_prebuilt = os.environ.get('USE_PREBUILT_CHIBIOS', '').lower() in ['1', 'true', 'yes']
    use_prebuilt_chibios = env_use_prebuilt or USE_PREBUILT_CHIBIOS
    prebuilt_libch = os.path.join(bld.env.SRCROOT, 'modules/ChibiOS/libch.a')
    
    if use_prebuilt_chibios and os.path.exists(prebuilt_libch):
        # Use prebuilt libch.a - copy to build directory and skip compilation
        print("Using prebuilt libch.a from: %s" % prebuilt_libch)
        bldpath_libch = os.path.join(bld.env.BUILDROOT, 'modules/ChibiOS/libch.a')
        os.makedirs(os.path.dirname(bldpath_libch), exist_ok=True)
        if not os.path.exists(bldpath_libch):
            shutil.copy2(prebuilt_libch, bldpath_libch)
        
        # Create a dummy task for build dependencies
        ch_task = bld(
            rule="echo 'Using prebuilt libch.a'",
            group='dynamic_sources',
            source=[],
            target=bld.bldnode.find_or_declare('modules/ChibiOS/libch.a')
        )
        ch_task.name = "ChibiOS_lib_prebuilt"
    else:
        # Build libch.a from ChibiOS sources
    ...
    bld.env.LIBPATH += ['../../module/ChibiOS/'] 
    if bld.env.ENABLE_CRASHDUMP:
