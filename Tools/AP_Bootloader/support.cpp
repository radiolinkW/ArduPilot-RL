...
static const uint8_t *flash_base = (const uint8_t *)(0x00000000 + (FLASH_BOOTLOADER_LOAD_KB + APP_START_OFFSET_KB)*1024U);
...
{
    uint32_t reserved = 0;
    num_pages = hc32_flash_getnumpages();

    /* flash default mode initialize */
    hc32_flash_config();
    /*
    ...
    while (reserved < (FLASH_BOOTLOADER_LOAD_KB + APP_START_OFFSET_KB) * 1024U &&
        ...
        reserved += hc32_flash_getpagesize(flash_base_page);
        flash_base_page++;
    }
    /*
    ...
    while (reserved < FLASH_RESERVE_END_KB * 1024U) {
        reserved += hc32_flash_getpagesize(num_pages-1);
        num_pages--;
    }
}
...
{
    return hc32_flash_write(uint32_t(flash_base+offset), &v, sizeof(v));
}

bool flash_func_write_words(uint32_t offset, uint32_t *v, uint8_t n)
{
    return hc32_flash_write(uint32_t(flash_base+offset), v, n*sizeof(*v));
}
...
uint32_t flash_func_sector_size(uint32_t sector)
{
    ...
    return hc32_flash_getpagesize(flash_base_page+sector);
}

bool flash_func_erase_sector(uint32_t sector)
{
    if (!hc32_flash_ispageerased(flash_base_page+sector)) {
        return hc32_flash_erasepage(flash_base_page+sector);
    }
    return true;
}
...
{
	uint32_t idcode = M4_EFM->CHIPID;
    int32_t mcuid = idcode & DEVID_MASK;
    //uint16_t revid = ((idcode & REVID_MASK) >> 16);

    mcu_des_t des = mcu_descriptions[HC32_UNKNOWN];
