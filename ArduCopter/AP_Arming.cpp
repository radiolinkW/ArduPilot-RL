...
bool AP_Arming_Copter::board_voltage_checks(bool display_failure)
{
    ...
    if (check_enabled(ARMING_CHECK_VOLTAGE)) {
        if (copter.battery.has_failsafed()) {
            check_failed(ARMING_CHECK_VOLTAGE, display_failure, "009-Battery failsafe");
            return false;
        }
    }
...
bool AP_Arming_Copter::gps_checks(bool display_failure)
{
    ...
    if ((copter.gps.num_sensors() > 0) && (copter.gps.get_hdop() > copter.g.gps_hdop_good)) {
        check_failed(ARMING_CHECK_GPS, display_failure, "002-High GPS HDOP");
        AP_Notify::flags.pre_arm_gps_check = false;
        return false;
    }
...
bool AP_Arming_Copter::mandatory_gps_checks(bool display_failure)
{
    ...
    if (mode_requires_gps || require_location == RequireLocation::YES) {
        if (!copter.position_ok()) {
            ...
            check_failed(display_failure, "002-Need Position Estimate");
            return false;
        }
    ...
    if (ahrs.get_filter_status(filt_status)) {
        if (filt_status.flags.gps_glitching) {
            check_failed(display_failure, "002-GPS glitching");
            return false;
        }
    }
    ...
    if (copter.g.fs_ekf_thresh > 0.0f) {
        ...
        if (mag_variance.length() >= copter.g.fs_ekf_thresh) {
            check_failed(display_failure, "005-EKF compass variance");
            return false;
        }
        if (pos_variance >= copter.g.fs_ekf_thresh) {
            check_failed(display_failure, "002-EKF position variance");
            return false;
        }
...
bool AP_Arming_Copter::arm_checks(AP_Arming::Method method)
{
    ...
    // always check if inertial nav has started and is ready
    if (!ahrs.healthy()) {
        check_failed(true, "011-AHRS not healthy");
        return false;
    }
    ...
    if (!ahrs.using_noncompass_for_yaw()) {
        ...
        if (!_compass.healthy()) {
            check_failed(true, "003-Compass not healthy");
            return false;
        }
    }
#endif
    ...
    if (!copter.flightmode->allows_arming(method)) {
        check_failed(true, "012-%s mode not armable", copter.flightmode->name());
        return false;
    }
    ...
    if (check_enabled(ARMING_CHECK_INS)) {
        if (degrees(acosf(ahrs.cos_roll()*ahrs.cos_pitch()))*100.0f > copter.aparm.angle_max) {
            check_failed(ARMING_CHECK_INS, true, "010-Leaning");
            return false;
        }
    }
    ...
    if (check_enabled(ARMING_CHECK_RC)) {
        ...
        if (!((AP_Arming::method_is_GCS(method) || method == AP_Arming::Method::SCRIPTING) && copter.flightmode->allows_GCS_or_SCR_arming_with_throttle_high())) {
            ...
            if (copter.get_pilot_desired_climb_rate(copter.channel_throttle->get_control_in()) > 0.0f) {
                check_failed(ARMING_CHECK_RC, true, "006-%s too high", rc_item);
                return false;
            }
