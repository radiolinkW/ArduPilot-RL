...
#if !defined(HAL_DISABLE_I2C_MAGS_BY_DEFAULT) || defined(HAL_USE_I2C_MAG_VCM5883L)
    //external i2c bus
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_VCM5883L, AP_Compass_VCM5883L::probe(GET_I2C_DEVICE(i, HAL_COMPASS_VCM5883L_I2C_ADDR),
                    true, HAL_COMPASS_VCM5883L_ORIENTATION_EXTERNAL));
    }

    // internal i2c bus
    if (all_external) {
        // only probe VCM5883L on internal if we are treating internals as externals
        FOREACH_I2C_INTERNAL(i) {
            ADD_BACKEND(DRIVER_VCM5883L, AP_Compass_VCM5883L::probe(GET_I2C_DEVICE(i, HAL_COMPASS_VCM5883L_I2C_ADDR),
                        all_external,
                        all_external?HAL_COMPASS_VCM5883L_ORIENTATION_EXTERNAL:HAL_COMPASS_VCM5883L_ORIENTATION_INTERNAL));
        }
    }
#endif
