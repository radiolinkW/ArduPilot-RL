...
uint16_t AP_GyroFFT::run_cycle()
{
    ...
    _output_cycle_micros = AP_HAL::Calclate_dt(_thread_state._last_output_us[_update_axis],now);
...
{
    uint32_t now = AP_HAL::micros();
    const float slew = MIN(1.0f, AP_HAL::Calclate_dt(now,_global_state._last_output_us[axis])
...
void AP_GyroFFT::write_log_messages()
{
    ...
    // output at 1hz
    if (AP_HAL::Calclate_dt(now,_last_output_ms) > 1000) {
