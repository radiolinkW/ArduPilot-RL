...
void AP_RCProtocol_CRSF::_process_byte(uint8_t byte)
{
    ...
    if (_frame_ofs > 0 && AP_HAL::Calclate_dt(now,_start_frame_time_us) > CRSF_FRAME_TIMEOUT_US) {
        _frame_ofs = 0;
    }
...
void AP_RCProtocol_CRSF::update(void)
{
    ...
    if (_last_frame_time_us > 0 && (!get_rc_frame_count() || !is_tx_active())
        && AP_HAL::Calclate_dt(now,_last_frame_time_us) > CRSF_INTER_FRAME_TIME_US_250HZ) {
