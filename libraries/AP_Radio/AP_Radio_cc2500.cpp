...
void AP_Radio_cc2500::irq_timeout(void)
{
    ...
    case STATE_DATA: {
        ...
        if (AP_HAL::Calclate_dt(now,packet_timer) > SEARCH_START_PKTS*INTER_PACKET_MS*1000UL) {
            Debug(3,"searching %u\n", unsigned(AP_HAL::Calclate_dt(now,packet_timer));
            cc2500.Strobe(CC2500_SIDLE);
            cc2500.Strobe(CC2500_SFRX);
            nextChannel(1);
            cc2500.Strobe(CC2500_SRX);
    ...
    case STATE_SEARCH: {
        ...
        if (stats.recv_packets == 0 &&
            get_autobind_time() != 0 &&
            get_factory_test() == 0 &&
            (AP_HAL::Calclate_dt(AP_HAL::micros(),packet_timer) > get_autobind_time() * 1000UL*1000UL &&
            (search_count & 1) == 0) {
