...
void AP_Radio_cypress::process_packet(const uint8_t *pkt, uint8_t len)
{
    if (len == 16) {
        ...
        if (ok) {
            uint32_t packet_dt_us = AP_HAL::Calclate_dt(now,dsm.last_recv_us);
...
{
    uint32_t now = AP_HAL::micros();
    uint32_t dt = AP_HAL::Calclate_dt(now,dsm.last_recv_us);
    const uint32_t cycle_time = dsm.pkt_time1 + dsm.pkt_time2;
    uint8_t next_channel;


    if (state == STATE_BIND) {
        if (AP_HAL::Calclate_dt(now,dsm.last_chan_change_us) > 15000) {
    ...
    if (get_autobind_time() != 0 &&
        dsm.last_recv_us == 0 &&
		AP_HAL::Calclate_dt(now,dsm.last_autobind_send) > 300*1000UL &&
        ...
        Debug(3,"recv autobind %u\n", unsigned(AP_HAL::Calclate_dt(now,dsm.last_autobind_send));
    ...
    if (is_DSM2()) {
        if (AP_HAL::Calclate_dt(now,dsm.last_recv_us) > 5000000) {
            dsm2_start_sync();
        }
    }
