...
void AP_Radio_beken::irq_handler(uint32_t when)
{
    ...
    if (bReply) {
        ...
        uint32_t delta = chTimeUS2I(800 + AP_HAL::Calclate_dt(next_switch_us,now)); // Do not use US2ST since that will overflow 32 bits
        chSysLock();
...
void AP_Radio_beken::irq_timeout(uint32_t when)
{
    ...
    if (beken.bkReady) { // We are not reinitialising the chip in the main thread
        ...
        if (beken.fcc.fcc_mode) {
            ...
            uint32_t dt = AP_HAL::Calclate_dt(when,synctm.rx_time_us);
