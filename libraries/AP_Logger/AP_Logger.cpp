...
void AP_Logger::io_thread(void)
{
    ...
    while (true) {
        ...
        if (AP_HAL::Calclate_dt(now,last_run_us) < 1000) {
            delay = MAX(1000 - AP_HAL::Calclate_dt(now,last_run_us), delay);
        }
        ...
        if (AP_HAL::Calclate_dt(now,last_stack_us) > 100000U) {
            last_stack_us = now;
        ...
        if (!done_crash_dump_save &&
        	AP_HAL::Calclate_dt(now,last_crash_check_us) > 5000000U) {
...
bool AP_Logger::log_while_disarmed(void) const
{
    ...
    if (!hal.util->get_soft_armed() && arm_change_ms != 0 && AP_HAL::Calclate_dt(now,arm_change_ms) < persist_ms) {
        return true;
    }

    // keep logging for HAL_LOGGER_ARM_PERSIST seconds after an arming failure
    if (_last_arming_failure_ms && AP_HAL::Calclate_dt(now,_last_arming_failure_ms) < persist_ms) {
        return true;
    }
