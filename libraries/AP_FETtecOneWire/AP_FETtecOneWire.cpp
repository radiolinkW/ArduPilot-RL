...
bool AP_FETtecOneWire::transmit(const uint8_t* bytes, const uint8_t length)
{
    ...
    if (AP_HAL::Calclate_dt(now,_last_transmit_us) < _min_fast_throttle_period_us) {
...
#if HAL_WITH_ESC_TELEM
        if ( AP_HAL::Calclate_dt(now,esc.last_telem_us) > max_telem_interval_us) {
            no_telem++;
        }
#endif
    }
...
void AP_FETtecOneWire::update()
{
    ...
    if (!hal.util->get_soft_armed()) {
        ...
        for (uint8_t i=0; i<_esc_count; i++) {
            auto &esc = _escs[i];
            if (!esc.telem_requested || AP_HAL::Calclate_dt(now,esc.last_telem_us) < 1000000U ) {
                // telem OK
                continue;
            }
            _running_mask &= ~(1U << esc.servo_ofs);
    ...
    if (AP_HAL::Calclate_dt(now,_last_transmit_us) < 700U) {
        // in case the SRV_Channels::push() is running at very high rates, limit the period
        // this function gets executed because FETtec needs a time gap between frames
        _period_too_short++;
        return;
