...
#endif
#if STORAGE_NUM_AREAS >= 21  		  // 4+6+5+3+3=21
    { StorageParam,    32768, 1280},  //27506+5262 = 32768
    { StorageMission,  34048, 9842}, // 32768+1280 = 34048  //2506+2132+5204+9842 = 19684
    { StorageParamBak, 43890, 5262}, // 32768 - 1280 - 9842 = 5262
#endif
#if STORAGE_NUM_AREAS >= 24  		  // 4+6+5+3+3+3=24
    { StorageParam,    49152, 1280},  //
    { StorageMission,  50432, 4921}, //
    { StorageParamBak, 55353, 1991}, //
#endif
#if STORAGE_NUM_AREAS >= 27  		  // 4+6+5+3+3+3=24
    { StorageParam,    57344, 1280},  //
    { StorageMission,  58624, 4921}, //
    { StorageParamBak, 63545, 1991}, //
#endif
#if STORAGE_NUM_AREAS >= 30
    { StorageParam,    65536, 1280},
    { StorageMission,  66816, 4921},
    { StorageParamBak, 71737, 1991},
#endif
};
...
*/
bool StorageAccess::read_block(void *data, uint32_t addr, size_t n) const
{
    uint8_t *b = (uint8_t *)data;
    for (uint8_t i=0; i<STORAGE_NUM_AREAS; i++) {
        const StorageManager::StorageArea &area = StorageManager::layout[i];
        uint32_t length = area.length;
...
/*
  ...
  for the storage type of this StorageAccess object
*/
bool StorageAccess::write_block(uint32_t addr, const void *data, size_t n) const
{
...
/*
  read a byte
 */
uint8_t StorageAccess::read_byte(uint32_t loc) const
{
    uint8_t v;
...
/*
  read 16 bit value
 */
uint16_t StorageAccess::read_uint16(uint32_t loc) const
{
    uint16_t v;
    read_block(&v, loc, sizeof(v));
    return v;
...
/*
  read 32 bit value
 */
uint32_t StorageAccess::read_uint32(uint32_t loc) const
{
    uint32_t v;
    read_block(&v, loc, sizeof(v));
    return v;
...
/*
  read a float
 */
float StorageAccess::read_float(uint32_t loc) const
{
    float v;
    read_block(&v, loc, sizeof(v));
    return v;
...
/*
  write a byte
 */
void StorageAccess::write_byte(uint32_t loc, uint8_t value) const
{
...
/*
  write a uint16
 */
void StorageAccess::write_uint16(uint32_t loc, uint16_t value) const
{
    write_block(loc, &value, sizeof(value));
}
...
/*
  write a uint32
 */
void StorageAccess::write_uint32(uint32_t loc, uint32_t value) const
{
    write_block(loc, &value, sizeof(value));
}
...
/*
  write a float
 */
void StorageAccess::write_float(uint32_t loc, float value) const
{
    write_block(loc, &value, sizeof(value));
}
...
bool StorageAccess::copy_area(const StorageAccess &source) const
{
    ...
    uint32_t total = MIN(source.size(), size());
    uint32_t ofs = 0;
    while (total > 0) {
        uint8_t block[32];
