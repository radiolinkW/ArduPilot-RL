...
void AP_Scheduler::run(uint32_t time_available)
{
    ...
    for (uint8_t i=0; i<_num_tasks; i++) {
        ...
        now = AP_HAL::micros();
        uint32_t time_taken = AP_HAL::Calclate_dt(now,_task_time_started);
        bool overrun = false;
...
{
    uint32_t dt = AP_HAL::Calclate_dt(AP_HAL::micros(), _task_time_started);
...
void AP_Scheduler::loop()
{
    ...
    if (_loop_timer_start_us == 0) {
        ...
        _last_loop_time_s = AP_HAL::Calclate_dt(sample_time_us,_loop_timer_start_us) * 1.0e-6;
    }
    ...
    const uint32_t loop_tick_us = AP_HAL::Calclate_dt(now,sample_time_us);
    ...
    // check loop time
    perf_info.check_loop_time(AP_HAL::Calclate_dt(sample_time_us,_loop_timer_start_us));
