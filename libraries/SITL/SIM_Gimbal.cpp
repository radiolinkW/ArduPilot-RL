...
void Gimbal::update(void)
{
    ...
    float delta_t = AP_HAL::Calclate_dt(now_us,last_update_us) * 1.0e-6f;
...
void Gimbal::send_report(void)
{
     ...
     */
    uint32_t now_us = AP_HAL::micros();
    if (AP_HAL::Calclate_dt(now_us,last_report_us) > reporting_period_ms*1000UL) {
        mavlink_gimbal_report_t gimbal_report;
        float delta_time = AP_HAL::Calclate_dt(now_us,last_report_us) * 1.0e-6f;
