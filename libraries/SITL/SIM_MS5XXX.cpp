...
void MS5XXX::update(const class Aircraft &aircraft)
{
    ...
    case State::COLD_WAIT:
        // 1ms to do anything....
        if (AP_HAL::Calclate_dt(now_us,command_start_us) < 1) {
            break;
        }
    ...
    case State::RESET_WAIT:
        ...
        if (AP_HAL::Calclate_dt(now_us,command_start_us) > 2000) {
            reset();
            state = State::RUNNING;
            break;
        }
    ...
    case State::CONVERSION_D2_WAIT:
        // driver allows for 10ms for a conversion to happen
        if (AP_HAL::Calclate_dt(now_us,command_start_us) > conversion_time_osr_1024_us) {
            convert_D2();
            state = State::RUNNING;
            break;
        }
