...
void LORD::update(void)
{
    ...
    if (AP_HAL::Calclate_dt(now,last_imu_pkt_us) >= us_between_imu_packets) {
        last_imu_pkt_us = now;
        send_imu_packet();
    }

    if (AP_HAL::Calclate_dt(now,last_gnss_pkt_us) >= us_between_gnss_packets) {
        last_gnss_pkt_us = now;
        send_gnss_packet();
    }

    if (AP_HAL::Calclate_dt(now,last_filter_pkt_us) >= us_between_filter_packets) {
