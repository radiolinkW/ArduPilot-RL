...
private:
    ...
    static const uint8_t dshot_post = 1;
    ...
    struct pwm_group {
        ...
        const hc32_dma_channel_t *Per_dma;
        uint8_t dma_per_channel_id;
#if AP_HAL_SHARED_DMA_ENABLED
        Shared_DMA *dma_handle;
#endif
        dmar_uint_t *dma_buffer;
        dmar_uint_t *period_buffer;
        uint16_t period_buffer_len;
        ...
        en_event_src_t      dma_trigger_src;
#if HAL_SERIALLED_ENABLED
        M4_TMRA_TypeDef *cmp_tima;
        // serial LED support
        ...
        struct {
            ...
            const hc32_dma_channel_t *ic_dma[4];
        ...
        bool can_send_dshot_pulse() const {
          return is_dshot_protocol(current_mode) && AP_HAL::Calclate_dt(AP_HAL::micros(),last_dmar_send_us) > (dshot_pulse_time_us + 50);
        }
    ...
    float BLS_Scaling = 1.025;
    float BL32_Scaling = 1.004;
    // dshot period
    ...
    void Start_Cmp_Timer(pwm_group &group);
    ...
    */
    ...
    void fill_DMA_period_buffer(pwm_group &group);
    ...
    static void dma_up_irq_callback(void *p, dma_int_status_t flags);
    static void dma_unlock(virtual_timer_t* vt, void *p);
    static void dma_period_irq_callback(void *p, dma_int_status_t flags);
    ...
    void fill_DMA_buffer_byte(dmar_uint_t *buffer, uint8_t stride, uint8_t b, uint16_t pconrx, uint32_t bitval);
