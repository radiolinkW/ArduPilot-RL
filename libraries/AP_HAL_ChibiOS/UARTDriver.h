...
public:
    ...
    // request information on uart I/O for one uart    void uart_info(ExpandingString &str, StatsTracker &stats, const uint32_t dt_ms) override;
...
#ifndef HAL_UART_NODMA
    const hc32_dma_channel_t* rxdma;
    const hc32_dma_channel_t* txdma;
...
#endif
    static void rxbuff_full_irq(void* self, dma_int_status_t   flags);
    static void tx_complete(void* self, dma_int_status_t   flags);
