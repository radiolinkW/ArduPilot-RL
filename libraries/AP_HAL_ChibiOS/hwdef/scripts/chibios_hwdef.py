...
parser = argparse.ArgumentParser("chibios_pins.py")  #鍒涘缓涓�涓猚hibios_pins.py瀵硅薄锛�"chibios_pins.py"鍙湁鍙棤锛�
parser.add_argument(
...
parser.add_argument(
    '--params', type=str, default=None, help='user default params path') #娣诲姞鍙傛暟

args = parser.parse_args() #瑙ｆ瀽鍙傛暟

# output variables for each pin
af_labels = ['USART', 'UART', 'SPI', 'I2C', 'SDIO', 'SDMMC', 'OTG', 'JT', 'TIM', 'CAN', 'USBFS_VBUS']

default_gpio = ['INPUT', 'OFF']


vtypes = ['STATE', 'DIR', 'OTYPE', 'DRV', 'LATCH', 'PU', 'INVERT', 'ITYPE', 'EXINT', 'ATTR']
...
class generic_pin(object):
    ...
    def get_STATE(self):
        '''return one of SET, RESET wrapped in PIN_ODR macro'''
        return "PIN_STATE_%s" % (self.get_STATE_value())

    def get_OTYPE_value(self):
        '''return one of CMOS, NMOS'''
        v = 'CMOS'
    ...
    def get_OTYPE(self):
        '''return one of CMOS, NMOS'''
        return "PIN_OTYPE_%s" % (self.get_OTYPE_value())

    def get_DIR_value(self):
        '''return one of  OUTPUT,INPUT'''
        if self.type == 'OUTPUT':
            v = "OUTPUT"
        elif self.is_CS():
            v = "OUTPUT"
    ...
    def get_DIR(self):
        ...
        return "PIN_DIR_%s" % (self.get_DIR_value())

    def get_DRV_value(self):
        '''return one of  LOW, MID, HIGH'''
        # on HC32F4 these speeds correspond to LOW, MID and HIGH
        values = ['LOW', 'MID', 'HIGH']
        v = 'MID'
        for e in self.extra:
            if e in values:
    ...
    def get_DRV_int(self):
        '''return value from 0 to 2 for speed'''
        values = ['LOW', 'MID', 'HIGH']
        v = self.get_DRV_value()
        if v not in values:
    ...
    def get_DRV(self):
        '''return one of LOW, MID, HIGH'''
        return "PIN_DRV_%s" % (self.get_DRV_value())

    def get_LATCH_value(self):
        values = ['OFF', 'ON']
        v = 'OFF'
        return v

    def get_LATCH(self):
        '''return one of OFF, ON wrapped in PIN_PUPDR_ macro'''
        return "PIN_LATCH_%s" % (self.get_LATCH_value())

    def get_PU_value(self):
        '''return one of OFF, ON,'''
        values = ['OFF', 'ON']
        v = 'OFF'
        if self.is_CS():
            v = "ON"
    ...
    def get_PU(self):
        '''return one of OFF, ON wrapped in PIN_PUPDR_ macro'''
        return "PIN_PU_%s" % (self.get_PU_value())
    
    def get_INVERT_value(self):
        values = ['OFF', 'ON']
        v = 'OFF'
        return v
    ...
    def get_INVERT(self):
        '''return one of OFF, ON wrapped in PIN_PUPDR_ macro'''
        return "PIN_INVERT_%s" % (self.get_INVERT_value())

    def get_ITYPE_value(self):
        values = ['SMT', 'CMOS']
        values_miso = ['SPI1_MISO', 'SPI2_MISO', 'SPI3_MISO','SPI4_MISO', 'SPI5_MISO', 'SPI6_MISO']
        v = 'SMT'
        if self.label in values_miso:
            v = 'CMOS'
        return v
        
    def get_ITYPE(self):
        '''return one of SMT, CMOS wrapped in PIN_PUPDR_ macro'''
        return "PIN_ITYPE_%s" % (self.get_ITYPE_value())

    def get_EXINT_value(self):
        values = ['OFF', 'ON']
        v = 'OFF'
        if self.type.startswith('SAFETY'):
            v = 'ON'
        return v

    def get_EXINT(self):
        '''return one of SMT, CMOS wrapped in PIN_PUPDR_ macro'''
        return "PIN_EXINT_%s" % (self.get_EXINT_value())

    def get_ATTR_value(self):
        values = ['DIGITAL', 'ANALOG'] 
        v = "DIGITAL"
        if self.type.startswith('ADC'):  #鍒ゆ柇瀛楃涓叉槸鍚︿互ADC寮�澶�
            v = "ANALOG"
        elif self.type.startswith('OTG1'):
            v = "ANALOG"
        return v

    def get_ATTR(self):
        return "PIN_ATTR_%s" % (self.get_ATTR_value())
    ...
    def get_FUNC(self):
        ...
        return "GPIO_FUNC_%u" % (self.get_FUNC_value())

    def pal_modeline(self):
        '''return a mode line suitable for palSetModeLine()'''
        ret = 'PAL_HC32_STATE_' + self.get_STATE_value()
        ret += '|PAL_HC32_OTYPE_' + self.get_OTYPE_value()
        ret += '|PAL_HC32_DIR_' + self.get_DIR_value()
        ret += '|PAL_HC32_DRV_' % self.get_DRV_int()
        ret += '|PAL_HC32_LATCH_' + self.get_LATCH_value()
        ret += '|PAL_HC32_PU_' + self.get_PU_value()
        ret += '|PAL_HC32_INVERT_' + self.get_INVERT_value()
        ret += '|PAL_HC32_ITYPR_'+ self.get_ITYPE_value()
        ret += '|PAL_HC32_EXIT_'+ self.get_EXIT_value()
        ret += '|PAL_HC32_ATTR_'+ self.get_ATTR_value()
        af = self.get_AFIO_value()
        if af != 0:
            ret += '|PAL_HC32_ALTERNATE(%u)' % af
...
def make_line(label):
    ...
    if label in bylabel:
        p = bylabel[label]
        line = 'PAL_LINE(GPIO_PORT_%s,%uU)' % (p.port, p.pin)
    else:
        line = "0"
    return line
...
def get_flash_pages_sizes():
    global mcu_series
    if mcu_series.startswith('HC32F4'):
...
def write_mcu_config(f):
    ...
    if have_type_prefix('SDIO'):
        ...
        f.write('#define USE_POSIX\n\n')
        f.write('#define HAL_USE_SDC TRUE\n')
        f.write('#define HC32_SDC_USE_SDMMC2 TRUE\n')
        ...
        f.write('#define USE_POSIX\n\n')
        f.write('#define HAL_USE_SDC TRUE\n')
        f.write('#define HC32_SDC_USE_SDMMC1 TRUE\n')
        build_flags.append('USE_FATFS=yes')
        env_vars['WITH_FATFS'] = "1"
    elif has_sdcard_spi():
    ...
    if 'OTG1' in bytype:
        f.write('#define HC32_USB_USE_OTG1                  TRUE\n')
        f.write('#define HAL_USE_USB TRUE\n')
        f.write('#define HAL_USE_SERIAL_USB TRUE\n')
    if 'OTG2' in bytype:
        f.write('#define HC32_USB_USE_OTG2                  TRUE\n')
    ...
    # write any custom HC32 defines
    using_chibios_can = False
    for d in alllines:
        if d.startswith('HC32_'):
            f.write('#define %s\n' % d)
    ...
    f.write('#define FLASH_LOAD_ADDRESS 0x%08x\n' % (0x00000000 + flash_reserve_start*1024))
    ...
    if udid_start is None:
        f.write('#define UDID_START EFM_UQID0\n\n')
    else:
...
''')
    ...
    if not args.bootloader:
        f.write('''#define HC32_DMA_REQUIRED TRUE\n\n''')
...
# space to reserve for bootloader and storage at start of external flash
    ...
    flash_base = 0x00000000 + flash_reserve_start * 1024
    ...
    for dev in spidev:
        ...
        if not highspeed.endswith('*MHZ') and not highspeed.endswith('*KHZ'):
            error("Bad highspeed value %s in SPIDEV line %s" % (highspeed,
                                                                dev))
        cs_pin = bylabel[cs]
        pal_line = 'PAL_LINE(GPIO_PORT_%s,%uU)' % (cs_pin.port, cs_pin.pin)
        devidx = len(devlist)
        f.write(
...
def write_SPI_config(f):
    ...
    for dev in spi_list:
        ...
        f.write(
            '#define HAL_SPI%u_CONFIG { &SPID%u, %u, HC32_SPI_SPI%u_DMA_CHANNELS }\n'
            % (n, n, n, n))
...
def write_UART_config(f):
    ...
    if crash_uart is not None and get_config('FLASH_SIZE_KB', type=int) > 2048:
        ...
        f.write('#define IRQ_DISABLE_HAL_CRASH_SERIAL_PORT() NVIC_DisableIRQ(HC32_%s_TI_NUMBER);NVIC_DisableIRQ(HC32_%s_RI_NUMBER);NVIC_DisableIRQ(HC32_%s_EI_NUMBER);NVIC_DisableIRQ(HC32_%s_TCI_NUMBER)\n' % (crash_uart, crash_uart,crash_uart, crash_uart))
        f.write('#define RCC_RESET_HAL_CRASH_SERIAL_PORT()  SET_REG32_BIT(M4_PWC->FCG3, PWC_FCG3_%s); CLEAR_REG32_BIT(M4_PWC->FCG3, PWC_FCG3_%s)\n' % (crash_uart, crash_uart))
        f.write('#define HAL_CRASH_SERIAL_PORT_CLOCK USART_PCLK_DIV64\n')    
    for dev in uart_list:
        ...
        if dev.startswith('OTG2'):
            ...
            f.write(
                ...
                % (dev, n, n))
            f.write("HC32_%s_RX_DMA_CONFIG, HC32_%s_TX_DMA_CONFIG, %s, %s, %s, %s, " %
...
''')
    ...
    for dev in i2c_list:
        if not dev.startswith('I2C') or dev[3] not in "123456":
        ...
        f.write('''
#if defined(HC32_I2C_I2C%u_RX_DMA_CHANNEL) && defined(HC32_I2C_I2C%u_TX_DMA_CHANNEL)
#define HAL_I2C%u_CONFIG { &I2CD%u, %u, HC32_I2C_I2C%u_RX_DMA_CHANNEL, HC32_I2C_I2C%u_TX_DMA_CHANNEL, %s, %s }
#else
...
def parse_timer(str):
    ...
    result = re.match(r'TIM([a-zA-Z0-9])_([Z0-9])_PWM([a-zA-Z0-9])(N?)', str)
    #print("result = ",result)
    if result:      
        tim = result.group(1)
        tim_uint = int (result.group(2))
        chan = result.group(3)
        compl = result.group(4) == 'N'
        return (tim, tim_uint, chan, compl)
    else:
...
''')
    ...
    if rc_in is not None:
        (n, uint,chan, compl) = parse_timer(rc_in.label)
        if compl:
        ...
        if(n == '6'):
            if chan not in ['A', 'B']:
                error(
                    "Bad channel number, only channel A and B supported for RCIN")
            if(chan == 'A'):
                chan = 1
            else:
                chan = 2
            ...
            f.write('#define HC32_ICU_USE_TIM%s_%u TRUE\n' % (n,uint))
            f.write('#define RCIN_ICU_TIMER ICUD%u\n' % uint)
            f.write('#define RCIN_ICU_CHANNEL ICU_CHANNEL_%u\n' % chan)
            #f.write('#define HC32_RCIN_DMA_CHANNEL HC32_TIM_TIM%u_%u_PWM%s_DMA_CHANNEL\n' % (n, uint, chan))
            #f.write('#define HC32_RCIN_DMA_CHANNEL HC32_TIM_TIM%u__%u_PWM%s_DMA_CHAN\n' % (n, uint, chan))
            f.write('\n')
        else:
            if(chan != 3):
                error(
                    "Bad channel number, only channel 3 supported for RCIN")
            f.write('// RC input config\n')
            f.write('#define HAL_USE_ICU TRUE\n')
            f.write('#define HC32_ICU_USE_TIM%s_%u TRUE\n' % (n,uint))
            f.write('#define RCIN_ICU_TIMER ICUD%u\n' % 8+uint)
            f.write('#define RCIN_ICU_CHANNEL ICU_CHANNEL_%u\n' % int(chan))
            #f.write('#define HC32_RCIN_DMA_CHANNEL HC32_TIM_TIM%s_%u_PWM%u_DMA_CHANNEL\n' % (n, uint, int(chan)))
            #f.write('#define HC32_RCIN_DMA_CHANNEL HC32_TIM_TIM%s__%u_PWM%u_DMA_CHAN\n' % (n, uint, int(chan)))
            f.write('\n')

    if rc_in_int is not None:
        (n,uint, chan, compl) = parse_timer(rc_in_int.label)
        if compl:
        ...
        f.write('#define HC32_EICU_USE_TIM%s_%u TRUE\n' % (n,uint))
        f.write('#define RCININT_EICU_CHANNEL EICU_CHANNEL_%s\n' % chan)
        if(n == '6'):      
    ...
    if alarm is not None:
        (n, uint, chan, compl) = parse_timer(alarm.label)
        if compl:
            error("Complementary channel is not supported for ALARM %s" % alarm.label)
        f.write('\n')
        f.write('// Alarm PWM output config\n')
        f.write('#define HC32_PWM_USE_TIM%s_%u TRUE\n' % (n,uint))
        f.write('#define HC32_TIM%s_%u_SUPPRESS_ISR\n' % (n,uint))
        if(chan == 'A' or chan == 'B'):
            if(chan == 'A'):
                chan = 1
            else:
        ...
        if(n == '6'):
            ...
            { /* pwmGroup */ \\
              ...
              &PWMD%u /* PWMDriver* */ \\
                    ...
                    chan_mode[1], chan_mode[2], chan_mode[3], int(uint)))
        else:
            f.write('''#define HAL_PWM_ALARM \\
        { /* pwmGroup */ \\
          %u,  /* Timer channel */ \\
          ...
          &PWMD%u /* PWMDriver* */ \\
                    ...
                    (chan-1, pwm_clock, period, chan_mode[0],
                    chan_mode[1], chan_mode[2], chan_mode[3], int(uint)+8))
    # if ppmin is not None:
    #     f.write('\n')
    # else:
    #     f.write('\n')
    ...
    for t in pwm_timers:
        n = t[3:]
        f.write('#define HC32_PWM_USE_TIM%s TRUE\n' % n)
        f.write('#define HC32_TIM%s_SUPPRESS_ISR\n' % n)
    f.write('\n')
    ...
    for t in pwm_timers:
        ...
        for p in pwm_out:
            if p.type != t:
                continue
            (n, uint, chan, compl) = parse_timer(p.label)
            ...
            pal_lines[chan - 1] = 'PAL_LINE(GPIO_PORT_%s, %uU)' % (p.port, p.pin)
        groups.append('HAL_PWM_GROUP%u' % group)
        if (n == '6' and uint in [1-4]):
        ...
        if bidir is not None:
            ...
            hal_icu_def = '\n'
            for i in range(1,5):
                hal_icu_cfg += '{HAL_IC%u_CH%u_DMA_CONFIG},' % (num, i)
                hal_icu_def +='''#if defined(HC32_TIM_TIM%s_%u_CH%u_DMA_CHANNEL)
# define HAL_IC%u_CH%u_DMA_CONFIG true, HC32_TIM_TIM%s_%u_CH%u_DMA_CHANNEL
#else
# define HAL_IC%u_CH%u_DMA_CONFIG false, 0
#endif
''' % (n, uint, i, num, i, n, uint, i, num, i)
            hal_icu_cfg += '},  \\'


        f.write('''# define HAL_PWM%u_DMA_CONFIG false, 0, 0\n''' % (num))
    ...
    if have_complementary:
        f.write('#define HC32_PWM_USE_ADVANCED TRUE\n')
...
def write_ADC_config(f):
    ...
    vdd = get_config('HC32_VDD', default='330U')
    if vdd[-1] == 'U':
        vdd = vdd[:-1]
    vdd = float(vdd) * 0.01
...
def write_GPIO_config(f):
    ...
    for (gpio, pwm, port, pin, p, enabled) in gpios:
        f.write('#define HAL_GPIO_LINE_GPIO%u PAL_LINE(GPIO_PORT_%s, %2uU)\n' % (gpio, port, pin))
    f.write('#define HAL_GPIO_PINS { \\\n')
    for (gpio, pwm, port, pin, p, enabled) in gpios:
        f.write('{ %3u, true, %2u, PAL_LINE(GPIO_PORT_%s, %2uU)}, /* %s */ \\\n' %
                (gpio, pwm, port, pin, p))
    ...
    for l in sorted(list(set(bylabel.keys()))):
        ...
        if label == last_label:
            continue
        last_label = label
        f.write('#define HAL_GPIO_PIN_%-20s PAL_LINE(GPIO_PORT_%s,%uU)\n' %
                (label, p.port, p.pin))
    f.write('\n')
...
def write_peripheral_enable(f):
    ...
    for type in sorted(list(bytype.keys()) + list(alttype.keys())):
        if type.startswith('USART') or type.startswith('UART'):
            dstr = 'HC32_SERIAL_USE_%-6s' % type
            f.write('#ifndef %s\n' % dstr)
            f.write('#define %s TRUE\n' % dstr)
            f.write('#endif\n')
        if type.startswith('SPI'):
            f.write('#define HC32_SPI_USE_%s                  TRUE\n' % type)
        if type.startswith('OTG'):
            f.write('#define HC32_USB_USE_%s                  TRUE\n' % type)
        if type.startswith('I2C'):
            f.write('#define HC32_I2C_USE_%s                  TRUE\n' % type)
...
#define PAL_HC32_SPEED(n) ((n&3U)<<3U)
#define PAL_HC32_HIGH     0x8000U
...
* Please refer to the HC32 Reference Manual for details.
*/
#define PIN_DIR_INPUT           0U
#define PIN_DIR_OUTPUT          0x0002U
...
''')    
    ...
    for port in sorted(ports):
        if pincount[port] == 0:
           '''
            # handle blank ports
            for vtype in vtypes:                
                f.write("#define VAL_GPIO%s_%s_%-7s             0x0\n" % (port,pin,vtype))
            f.write("\n\n\n")
           '''
           continue
        else:
        ...
        for pin in range(pincount[port]): 
            p = portmap[port][pin]
            f.write("#define VAL_GPIO%s_%-7s (" % (p.port,p.pin))       
            first = True
            for vtype in vtypes:                
            ...
            if first:
                ...
                f.write("0")
            f.write(")\n\n")
            func = getattr(p, "get_" + "FUNC")
            v = func()
            f.write("#define GPIO%s_%s_FUNC       " % (p.port,p.pin))
            f.write(v)
            f.write("\n\n")
    write_alt_config(f)
    ...
    #print("dma_unassigned =",dma_unassigned)
...
def build_peripheral_list():
    ...
    for p in periph_pins:
        ...
        if type.startswith('TIM'):
            ...
            if not p.has_extra('ALARM') and not p.has_extra('RCININT') and not p.has_extra('RCIN'):
                # get the TIMn_UP DMA channels for DShot
                #label = p.type + '_UP'
                #if label not in peripherals and not p.has_extra('NODMA'):
                #    peripherals.append(label)
                ch_label = type
                (_, _, _, compl) = parse_timer(ch_label)
...
def process_line(line):
    ...
    a = shlex.split(line, posix=False) #浣跨敤闈� POSIX 妯″紡鍒嗗壊瀛楃涓�
