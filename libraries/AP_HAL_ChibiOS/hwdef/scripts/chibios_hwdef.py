...
class ChibiOSHWDef(object):
    ...
    hc32_vtypes = ['STATE', 'DIR', 'OTYPE', 'DRV', 'LATCH', 'PU', 'INVERT', 'ITYPE', 'EXINT', 'ATTR']
    af_labels = ['USART', 'UART', 'SPI', 'I2C', 'SDIO', 'SDMMC', 'OTG', 'JT', 'TIM', 'CAN', 'QUADSPI', 'OCTOSPI', 'ETH', 'MCO', 'USBFS_VBUS']
    ...
    def __init__(self, quiet=False, bootloader=False, signed_fw=False, outdir=None, hwdef=[], default_params_filepath=None):
        ...
        self.vtypes = ['STATE', 'DIR', 'OTYPE', 'DRV', 'LATCH', 'PU', 'INVERT', 'ITYPE', 'EXINT', 'ATTR']
    ...
    def setup_mcu_type_defaults(self):
        ...
        if self.mcu_series.startswith("STM32H7") or self.mcu_series.startswith("STM32F7") or self.mcu_series.startswith("HC32F4") :
...
''')
    ...
    def write_mcu_config(self, f):
        ...
        if 'OTG1' in self.bytype:
            if self.get_mcu_config('HC32_OTG2_IS_OTG1', False) is not None:
                f.write('#define HC32_USB_USE_OTG2                  TRUE\n')
                f.write('#define HC32_OTG2_IS_OTG1                  TRUE\n')
            else:
                f.write('#define HC32_USB_USE_OTG1                  TRUE\n')
                #f.write('#define HC32_OTG2_IS_OTG1                  FALSE\n')
...
#define HC32_ETH_BUFFERS_EXTERN
...
''')
        ...
        if self.get_mcu_config('EXPECTED_CLOCKS', required=False):
            ...
            for mcu_clock, mcu_clock_speed in self.get_mcu_config('EXPECTED_CLOCKS'):
                if (mcu_clock == 'HC32_HCLK' or mcu_clock == 'HC32_SYS_CK') and clockrate:
...
#endif
#define HC32_FLASH_DISABLE_ISR 0
''')
...
''' % (ext_flash_base, ext_flash_length, instruction_ram_base, instruction_ram_length, ram0_start, ram0_len, ram1_start, ram1_len, ram2_start, ram2_len))  # noqa
    ...
    def write_SPI_config(self, f):
        ...
        for dev in self.spi_list:
            ...
            sck_line = 'PAL_LINE(GPIO_PORT_%s,%uU)' % (sck_pin.port, sck_pin.pin)
            f.write(
                '#define HAL_SPI%u_CONFIG { &SPID%u, %u, HC32_SPI_SPI%u_DMA_CHANNELS, %s }\n'
                % (n, n, n, n, sck_line))
...
''')
    ...
    def write_UART_config(self, f):
        ...
        if crash_uart is not None and self.get_config('FLASH_SIZE_KB', type=int) >= 2048:
            ...
            f.write('#define IRQ_DISABLE_HAL_CRASH_SERIAL_PORT() nvicDisableVector(HC32_%s_NUMBER)\n' % crash_uart)
            f.write('#define RCC_RESET_HAL_CRASH_SERIAL_PORT() rccReset%s(); rccEnable%s(true)\n' % (crash_uart, crash_uart))
            f.write('#define HAL_CRASH_SERIAL_PORT_CLOCK HC32_%sCLK\n' % crash_uart)
        ...
        for num, dev in enumerate(serial_list):
            ...
            if dev.startswith('OTG2'):
                ...
                f.write(
                    '#define HAL_%s_CONFIG {(BaseSequentialStream*) &SDU1, 1, true, false, 0, 0, false, 0, 0' % dev)  # noqa
                if have_low_noise:
                    f.write('false}\n')
                else:
                    f.write('}\n')
...
'''
    ...
    def parse_timer(self, str):
        '''parse timer channel string, i.e TIMA_3_PWM1'''
...
''')
        ...
        if rc_in is not None:
            (n, uint, chan, compl) = self.parse_timer(rc_in.label)
            if compl:
        ...
        if rc_in_int is not None:
            (n, uint, chan, compl) = self.parse_timer(rc_in_int.label)
            if compl:
            ...
            f.write('#define HC32_EICU_USE_TIM%u TRUE\n' % n)
        ...
        if alarm is not None:
            (n, uint, chan, compl) = self.parse_timer(alarm.label)
            if compl:
        ...
        for t in pwm_timers:
            ...
            for p in pwm_out:
                if p.type != t:
                    continue
                (n, uint, chan, compl) = self.parse_timer(p.label)
    ...
    def write_ADC_config(self, f):
        ...
        vdd = self.get_config('HC32_VDD', default='330U')
        if vdd[-1] == 'U':
            vdd = vdd[:-1]
        vdd = float(vdd) * 0.01
    ...
    def write_GPIO_config(self, f):
        ...
        for (gpio, pwm, port, pin, p, enabled) in gpios:
            f.write('#define HAL_GPIO_LINE_GPIO%u PAL_LINE(GPIO_PORT_%s,%uU)\n' % (gpio, port, pin))
        if len(gpios) > 0:
            f.write('#define HAL_GPIO_PINS { \\\n')
            for (gpio, pwm, port, pin, p, enabled) in gpios:
                f.write('{ %3u, %s, %2u, PAL_LINE(GPIO_PORT_%s,%uU)}, /* %s */ \\\n' %
    ...
    def write_peripheral_enable(self, f):
        ...
        for type in sorted(list(self.bytype.keys()) + list(self.alttype.keys())):
            ...
            if type.startswith('QUADSPI'):
                f.write('#define HC32_WSPI_USE_%s                 TRUE\n' % type)
...
#                    ret += "# End included file (%s)" % include_filepath
    ...
    def valid_type(self, ptype, label):
        ...
        patterns = [
            r'INPUT', r'OUTPUT', r'TIM([a-zA-Z0-9])', r'USART\d+', r'UART\d+', r'ADC\d+',
        ...
        m1 = re.match(r'TIM(a-zA-Z0-9)', ptype)
        m2 = re.match(r'TIM(a-zA-Z0-9)_CH\d+', label)
        ...
        if p is not None:
            ...
            for u in a[1:]:
                ...
                for line in self.alllines[:]:
                    if line.startswith('define') and u == line.split()[1] or line.startswith('HC32_') and u == line.split()[0]:  # noqa
