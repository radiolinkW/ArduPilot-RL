...
def check_possibility(periph, dma_channel, curr_dict, dma_map, check_list, cannot_use_stream, forbidden_map):
    global ignore_list
    if debug:
        print('............ Checking ', periph, dma_channel, 'without', cannot_use_stream)
            ...
            if curr_dict[other_periph] == dma_channel:
                ...
                for chan in dma_map[other_periph]:
                    ...
                    if channel != curr_dict[other_periph] and \
                        channel not in cannot_use_stream and \
                        check_possibility(other_periph, channel, curr_dict, dma_map, check_list,
                                          cannot_use_stream+[(dma_channel)], forbidden_map):
...
def generate_DMAMUX_map_mask(peripheral_list, channel_mask, noshare_list, dma_exclude, channel_ofs):
    ...
    if channel_ofs != 0:
        # add in channel_ofs to cope with STM32G4
...
def generate_DMAMUX_map(peripheral_list, noshare_list, dma_exclude, channel_ofs):
    ...
    map2 = generate_DMAMUX_map_mask(dmamux2_peripherals, 0xff, noshare_list, dma_exclude, channel_ofs)
...
#                        channel = (shared_channels[0][0], shared_channels[0][1])
#                        curr_dict[periph] = channel
#                        curr_dict[periph2] = channel
#                        forbidden_channels.append(channel)
#                        periphs.remove(periph)
#                        periphs.remove(periph2)
#                        print("Sharing channel %s for %s %s" % (channel, periph, periph2))
    ...
    for periph in periphs:
        ...
        for chan in dma_map[periph]:
            ...
            if check_possibility(periph, channel, curr_dict, dma_map,
                                 check_list, forbidden_channels, forbidden_map):
                curr_dict[periph] = channel
                if debug:
    ...
    for key in sorted(curr_dict.keys()):
        ...
        if curr_dict[key] == "HC32_DMA_CHANNEL_ID_ANY":
            f.write("#define %-30s HC32_DMA_CHANNEL_ID_ANY\n" % (chibios_dma_define_name(key)+'CHANNEL'))
                ...
                for ch in range(1,5):
                    ...
                    f.write("#define %-30s HC32_DMA_CHANNEL_ID(%u, %u)%s\n" %
                        (chibios_dma_define_name(chkey)+'CHANNEL', dma_controller,
                            curr_dict[key][1], shared))
        for chan in sorted(dma_map[key]):
    ...
    for u in range(1, 9):
        ...
        if dma_name(key) == 'BDMA':
            ...
            f.write('#define HC32_SPI_%s_DMA_CHANNELS SHARED_DMA_NONE, SHARED_DMA_NONE\n' % key)
        else:
