...
def check_possibility(periph, dma_channel, curr_dict, dma_map, check_list, forbidden_map):
    if debug:
        print('............ Checking ', periph, dma_channel, 'without')
    for other_periph in curr_dict:
        if other_periph != periph:
            if curr_dict[other_periph] == dma_channel:
                if other_periph in forbidden_map[periph]:
                    if debug:
                        print('.................... Forbidden', periph, other_periph)
                    return False
                if debug:
                    print('.................... Collision', other_periph, dma_channel)
                ignore_list.append(periph)
                check_str = "%s(%d,%d) %s(%d,%d)" % (
                    other_periph, curr_dict[other_periph][0],
                    curr_dict[other_periph][1], periph, dma_channel[0],
                    dma_channel[1])
                ...
                for chan in dma_map[other_periph]:
                    channel = (chan[0], chan[1])

                    if channel != curr_dict[other_periph] and \
                        channel not in  \
                        check_possibility(other_periph, channel, curr_dict, dma_map, check_list,forbidden_map):
                        curr_dict[other_periph] = channel
                        if debug:
                            print ('....................... Resolving', other_periph, channel)
                        return True
                if debug:
                    print ('....................... UnSolved !!!!!!!!', periph, dma_channel)                    
                return False
    if debug:
        print ('....................... Solved ..........', periph, dma_channel)    
    return True
...
def dmamux_channel(key):
    ...
    for p in DMAMUX2_peripherals:
        if key.find(p) != -1:
            return 'HC32_DMAMUX2_' + key
    # default to DMAMUX1
    return 'HC32_DMAMUX1_' + key
...
def chibios_dma_define_name(key):
    ...
    if key.startswith('ADC'):
        return 'HC32_ADC_%s_' % dma_key
    elif key.startswith('SPI'):
        return 'HC32_SPI_%s_' % dma_key
    elif key.startswith('I2C'):
        return 'HC32_I2C_%s_' % dma_key
    elif key.startswith('USART'):
        return 'HC32_USART_%s_' % dma_key
    elif key.startswith('SDIO'):
    	return 'HC32_SDC_%s_' % dma_key
    #elif key.startswith('TIM'):
        #return 'HC32_TIM_%s_' % dma_key
    else:
...
def generate_DMAMUX_map_mask(peripheral_list, channel_mask, noshare_list, dma_exclude, channel_ofs):
    '''
    generate a dma map suitable for a board with a DMAMUX

    In principle any peripheral can use any channel, but we need to
    ensure that a peripheral doesn't try to use the same channel as its
    ...
    for p in peripheral_list:
        ...
        for i in range(16):
            ...
            for ii in list(range(base,16)) + list(range(0,base)):
                ...
                if (dma,channel) in dma_map[p]:
                    # this peripheral is already using the channel
                    continue
                ...
                if other is not None and other in idsets and ii in idsets[other]:
                    ...
                    dma_map[other].remove((dma,channel))
                found = ii
                break
            if found is None:
                continue
    ...
    if channel_ofs != 0:
        # add in channel_ofs to cope with HC32G4
        for p in dma_map.keys():
            for (dma,channel) in dma_map[p]:
                map2 = []
                map2.append((dma,channel+channel_ofs))
                dma_map[p] = map2
...
def generate_DMAMUX_map(peripheral_list, noshare_list, dma_exclude, channel_ofs):
    '''
    ...
    map1 = generate_DMAMUX_map_mask(dmamux1_peripherals, 0xFFFF, noshare_list, dma_exclude, channel_ofs)
    ...
    map2 = generate_DMAMUX_map_mask(dmamux2_peripherals, 0x55, noshare_list, dma_exclude, channel_ofs)
    # translate entries from map2 to "DMA controller 3", which is used for BDMA
    for p in map2.keys():
        channels = []
        for (controller,channel) in map2[p]:
            channel.append((3,channel))
        map2[p] = channel
    both = map1
    both.update(map2)
    if debug:
...
def check_sharing(shared):
    ...
    for p in shared:
        # don't share USART RX with anything
        if (p.startswith("USART") or p.startswith("USART")) and p.endswith("_RX"):
...
def write_dma_header(f, peripheral_list, mcu_type, dma_exclude=[],
    ...
    if dma_map is None:
        ...
        dma_map = generate_DMAMUX_map(peripheral_list, noshare_list, dma_exclude, channel_ofs)
    ...
    for periph in peripheral_list:
        ...
        for chan in dma_map[periph]:
            if debug:
                print('........Possibility for', periph, chan)
            channel = (chan[0], chan[1])
            #print()
            if check_possibility(periph, channel, curr_dict, dma_map,
                                 check_list, forbidden_map):
                curr_dict[periph] = channel
                if debug:
                    print ('....................... Setting', periph, channel)
                assigned = True
                break
        if assigned == False:
    ...
    for k in curr_dict.keys():
        p = curr_dict[k]
        if not p in channel_assign:
            channel_assign[p] = [k]
        else:
            channel_assign[p].append(k)
    ...
    for periph in unassigned:
        ...
        for chan in dma_map[periph]:
            channel = (chan[0], chan[1])
            share_ok = True
            for periph2 in channel_assign[channel]:
                if not can_share(periph, noshare_list) or not can_share(periph2, noshare_list) or periph2 in forbidden_map[periph]:
                    share_ok = False
            if share_ok:
                share_possibility.append(channel)
        if share_possibility:
            # sort the possible sharings so minimise impact on high priority channels
            share_possibility = sorted(share_possibility, key=lambda x: get_sharing_priority(channel_assign[x], priority_list))
            # and take the one with the least impact (lowest value for highest priority channel share)
            channel = share_possibility[-1]
            if debug:
                print("Sharing %s on %s with %s" % (periph, channel,
                                                    channel_assign[channel]))
            curr_dict[periph] = channel
            channel_assign[channel].append(periph)
            unassigned_new.remove(periph)
    unassigned = unassigned_new

    for key in sorted(curr_dict.keys()):
        channel = curr_dict[key]
        if len(channel_assign[channel]) > 1:
            if not check_sharing(channel_assign[channel]):
                sys.exit(1)
    ...
    # produce a list of timers ordered by the DMA channelid of the UP channel
    ...
    def order_by_channelid(key):
        channel = curr_dict[key]
        tim = key[3:-4]
        if tim == "A":
            return(channel[0] * 8 + channel[1]) * 20 + 9 + int(key[5:-3])
        else:
            return (channel[0] * 8 + channel[1]) * 20 + int(key[5:-3])

    ordered_up_channels = sorted(ordered_up_channels, key=order_by_channelid)
    ordered_timers = []
    ...
    for key in sorted(curr_dict.keys()):
        ...
        shared = ''
        if len(channel_assign[channel]) > 1:
            shared = ' // shared %s' % ','.join(channel_assign[channel])
            if channel[0] in [1,2]:
                shared_set.add("(1U<<HC32_DMA_CHANNEL_ID(%u,%u))" % (channel[0],channel[1]))
        if curr_dict[key] == "HC32_CHANNEL_ID_ANY":
            f.write("#define %-30s HC32_CHANNEL_ID_ANY\n" % (chibios_dma_define_name(key)+'CHANNEL'))
            ...
            if dma_controller == 3:
                ...
                dma_controller = 1
            f.write("#define %-30s HC32_DMA_CHANNEL_ID(%u, %u)%s\n" %
                    (chibios_dma_define_name(key)+'CHANNEL', dma_controller,
            ...
            if have_DMAMUX and "_UP" in key:
                ...
                for ch in range(1,5):
                    ...
                    if chkey not in timer_ch_periph:
                        continue
                    f.write("#define %-30s HC32_DMA_CHANNEL_ID(%u, %u)%s\n" %
                        (chibios_dma_define_name(chkey)+'CHANNEL', dma_controller,
                            curr_dict[key][1], shared))
        for chan in dma_map[key]:
            if channel == (chan[0], chan[1]):
                if have_DMAMUX:
                    access = dmamux_channel(key)
                #else:
                #    access = chan[2]
                #f.write("#define %-30s %s\n" %
                 #       (chibios_dma_define_name(key)+'CHAN', access))
                if have_DMAMUX and "_UP" in key:
                ...
                #                access.replace('_UP', '_CH{}'.format(ch))))
                break

    f.write("\n// Mask of DMA channels which are shared\n")
    if len(shared_set) == 0:
        f.write("#define SHARED_DMA_MASK 0\n")
    else:
        f.write("#define SHARED_DMA_MASK (%s)\n" % '|'.join(list(shared_set)))

    # now generate USARTDriver.cpp DMA config lines
    f.write("\n\n// generated USART DMA configuration lines\n")
    for u in range(1, 9):
        key = None
        ...
        #else:
            dma_rx_chn = "HC32_USART_%s_RX_%s_CHAN" % (key, dma_name(key))
            dma_tx_chn = "HC32_USART_%s_TX_%s_CHAN" % (key, dma_name(key))

        f.write("#define HC32_%s_RX_DMA_CONFIG " % key)
        if key + "_RX" in curr_dict:
            f.write(
                "true, HC32_USART_%s_RX_%s_CHANNEL\n" % (key, dma_name(key)))
        else:
            f.write("false, 0, 0\n")
        f.write("#define HC32_%s_TX_DMA_CONFIG " % key)
        if key + "_TX" in curr_dict:
            f.write(
                "true, HC32_USART_%s_TX_%s_CHANNEL\n" % (key, dma_name(key)))
        else:
            f.write("false, 0, 0\n")

    # now generate SPI DMA channels lines
    ...
    for u in range(1, 9):
        if 'SPI%u_TX' % u in peripheral_list and 'SPI%u_RX' % u in peripheral_list:
            ...
            continue
        f.write('#define HC32_SPI_%s_DMA_CHANNELS HC32_SPI_%s_TX_%s_CHANNEL, HC32_SPI_%s_RX_%s_CHANNEL\n' % (
