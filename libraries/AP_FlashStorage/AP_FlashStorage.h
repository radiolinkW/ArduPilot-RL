...
private:
    ...
    static const uint16_t Backup_sector_num = HAL_STORAGE_SIZE/8192;
public:
    ...
    bool write(uint32_t offset, uint32_t length) WARN_IF_UNUSED;
...
#error "Unknown AP_FLASHSTORAGE_TYPE"
    ...
    struct block_header {
        uint16_t state:2;
        uint16_t block_num:14;
        //uint16_t num_blocks_minus_one:1;
    };
    ...
    bool all_zero(uint32_t ofs, uint32_t size) WARN_IF_UNUSED;
