...
bool AP_FlashStorage::init(void)
{
    ...
    for (uint8_t i=0; i<2; i++) {
        if (!flash_read((i*Backup_sector_num), 0, (uint8_t *)&header[i], sizeof(header[i]))) {
            return false;
        }
    ...
    if (states[0] == states[1]) {
        ...
        first_sector = Backup_sector_num;
    ...
    for (uint8_t i=0; i<2; i++) {
        ...
        if (states[sector] == SECTOR_STATE_IN_USE ||
            ...
            if (!load_sector(Backup_sector_num*sector)) {
                return erase_all();
            }
        }
    }
    ...
    for (uint8_t i=0; i<2; i++) {
        if (states[i] == SECTOR_STATE_FULL) {
            if (!erase_sector(((i*Backup_sector_num)), true)) {
                return false;
            }
            for(int j=((i*Backup_sector_num)+1);j<((i*Backup_sector_num)+Backup_sector_num);j++){
				if (!erase_sector(j, false)) {
					return false;
				}
            }
        }
...
bool AP_FlashStorage::protected_switch_full_sector(void)
{
    ...
    write_error = false;
    reserved_space = 0;
    switch_sector_times += 1;
    if (!write_all()) {
        return false;
    }
    if (!erase_sector(((current_sector ^ Backup_sector_num)), true)) {
        return false;
    }

    for(int i=((current_sector ^ Backup_sector_num)+1);i<((current_sector ^ Backup_sector_num)+Backup_sector_num);i++){
		if (!erase_sector(i, false)) {
			return false;
		}
    }

    if(switch_sector_times == 2){
    	switch_sector_times = 0;
    	return true;
    }
...
bool AP_FlashStorage::write(uint32_t offset, uint32_t length)
{
    if (write_error) {
        return false;
    }
...
//        uint8_t n = max_write;
        ...
        const uint32_t space_available = HAL_STORAGE_SIZE - write_offset;
        ...
        uint32_t block_ofs = blk.header.block_num*block_size;
        uint32_t block_nbytes = block_size;
...
bool AP_FlashStorage::load_sector(uint8_t sector)
{
    ...
    while (ofs < storage_size - sizeof(struct block_header)) {
        struct block_header header;
        ...
        case BLOCK_STATE_VALID: {
        	uint32_t block_nbytes = block_size;
            uint32_t block_ofs = header.block_num*block_size;
...
bool AP_FlashStorage::erase_all(void)
{
    ...
    for(int i = 0;i<(Backup_sector_num*2);i++){
    	if(i == Backup_sector_num){
			if (!erase_sector(Backup_sector_num, current_sector!=Backup_sector_num)) {
				   return false;
			   }
    	}
    	else{
			if (!erase_sector(i, current_sector!=0)) {
				   return false;
			   }
    	}
    }
...
bool AP_FlashStorage::write_all()
{
    ...
    for (uint32_t ofs=0; ofs<storage_size; ofs += max_write) {
...
bool AP_FlashStorage::all_zero(uint32_t ofs, uint32_t size)
{
    while (size--) {
        if (mem_buffer[ofs++] != 0) {
            return false;
...
bool AP_FlashStorage::switch_sectors(void)
{
    ...
    uint8_t new_sector = current_sector ^ Backup_sector_num;
