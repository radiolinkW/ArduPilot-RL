...
bool AP_ESC_Telem::are_motors_running(uint32_t servo_channel_mask, float min_rpm) const
{
    ...
    for (uint8_t i = 0; i < ESC_TELEM_MAX_ESCS; i++) {
        if (BIT_IS_SET(servo_channel_mask, i)) {
            ...
            if (now < rpmdata.last_update_us || AP_HAL::Calclate_dt(now,rpmdata.last_update_us) > ESC_RPM_CHECK_TIMEOUT_US) {
                return false;
            }
...
bool AP_ESC_Telem::get_rpm(uint8_t esc_index, float& rpm) const
{
    ...
    if (rpmdata.last_update_us > 0 && (now >= rpmdata.last_update_us)
        && (AP_HAL::Calclate_dt(now,rpmdata.last_update_us) < ESC_RPM_DATA_TIMEOUT_US)) {
        const float slew = MIN(1.0f, AP_HAL::Calclate_dt(now,rpmdata.last_update_us) * rpmdata.update_rate_hz * (1.0f / 1e6f));
...
bool AP_ESC_Telem::get_raw_rpm(uint8_t esc_index, float& rpm) const
{
    ...
    if (now < rpmdata.last_update_us || AP_HAL::Calclate_dt(now,rpmdata.last_update_us) > ESC_RPM_DATA_TIMEOUT_US) {
        return false;
    }
        ...
        for (uint8_t j=0; j<4; j++) {
            ...
            if (esc_id < ESC_TELEM_MAX_ESCS &&
                (now - _telem_data[esc_id].last_update_ms <= ESC_TELEM_DATA_TIMEOUT_MS ||
                		AP_HAL::Calclate_dt(now_us,_rpm_data[esc_id].last_update_us) <= ESC_RPM_DATA_TIMEOUT_US)) {
                all_stale = false;
                break;
            }
        }
