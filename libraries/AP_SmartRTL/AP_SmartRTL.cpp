...
void AP_SmartRTL::detect_simplifications()
{
    ...
    while (_simplify.stack_count > 0) { // while there is something to do
        ...
        if (AP_HAL::Calclate_dt(AP_HAL::micros(),start_time_us) > SMARTRTL_SIMPLIFY_TIME_US) {
            return;
        }
...
void AP_SmartRTL::detect_loops()
{
    ...
    while (AP_HAL::Calclate_dt(AP_HAL::micros(),start_time_us) < SMARTRTL_PRUNING_LOOP_TIME_US) {
