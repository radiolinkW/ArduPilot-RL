...
check_sample:
    ...
    now = AP_HAL::micros();
    _delta_time = AP_HAL::Calclate_dt(now,_last_sample_usec) * 1.0e-6f;
...
#if 0
    {
        ...
        if (counter++ == 400) {
            ...
            hal.console->printf("now=%lu _delta_time_sum=%lu diff=%ld\n",
                                ...
                                (long)AP_HAL::Calclate_dt(now,delta_time_sum));
        }
    }
#endif
