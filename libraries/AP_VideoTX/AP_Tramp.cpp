...
void AP_Tramp::process_requests()
{
    ...
    case TrampStatus::TRAMP_STATUS_OFFLINE: {
        ...
        } else if (AP_HAL::Calclate_dt(now,last_time_us) >= TRAMP_MIN_REQUEST_PERIOD_US) {
    ...
    case TrampStatus::TRAMP_STATUS_ONLINE_MONITOR_FREQPWRPIT: {
        ...
        if (retry_count > 0 && (AP_HAL::Calclate_dt(now,last_time_us) >= TRAMP_MIN_REQUEST_PERIOD_US)) {
            AP_VideoTX& vtx = AP::vtx();
        ...
        if (!configUpdateRequired) {
            ...
            if (AP_HAL::Calclate_dt(now,last_time_us) >= TRAMP_STATUS_REQUEST_PERIOD_US) {
    ...
    case TrampStatus::TRAMP_STATUS_ONLINE_CONFIG: {
        ...
        if (AP_HAL::Calclate_dt(now,last_time_us) >= TRAMP_MIN_REQUEST_PERIOD_US) {
