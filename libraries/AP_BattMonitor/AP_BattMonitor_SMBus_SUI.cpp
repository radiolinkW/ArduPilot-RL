...
void AP_BattMonitor_SMBus_SUI::update_health()
{
    ...
    _state.healthy =  AP_HAL::Calclate_dt(now,last_volt_read_us < AP_BATTMONITOR_SMBUS_TIMEOUT_MICROS) &&
        (AP_HAL::Calclate_dt(now,_state.last_time_micros) < AP_BATTMONITOR_SMBUS_TIMEOUT_MICROS);
}
