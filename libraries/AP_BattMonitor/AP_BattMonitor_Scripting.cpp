...
void AP_BattMonitor_Scripting::read()
{
    ...
    if (last_update_us == 0 || AP_HAL::Calclate_dt(AP_HAL::micros(),last_update_us) > AP_BATT_MONITOR_SCRIPTING_TIMEOUT_US) {
...
bool AP_BattMonitor_Scripting::handle_scripting(const BattMonitorScript_State &battmon_state)
{
    ...
    uint32_t dt_us = AP_HAL::Calclate_dt(now_us,last_update_us);
