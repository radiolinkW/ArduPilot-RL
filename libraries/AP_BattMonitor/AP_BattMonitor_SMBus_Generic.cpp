...
void AP_BattMonitor_SMBus_Generic::timer()
{
    ...
    if (!_cell_count_fixed) {
        if (_state.healthy) {
            ...
            if ( AP_HAL::Calclate_dt(tnow,_cell_count_check_start_us) > (SMBUS_CELL_COUNT_CHECK_TIMEOUT * 1e6)) {
    ...
    for (uint8_t i = 0; i < (_cell_count_fixed ? _cell_count : BATTMONITOR_SMBUS_NUM_CELLS_MAX); i++) {
        ...
        } else if ( AP_HAL::Calclate_dt(tnow,_last_cell_update_us[i]) > AP_BATTMONITOR_SMBUS_TIMEOUT_MICROS) {
            _state.cell_voltages.cells[i] = UINT16_MAX;
        }
    }

    // timeout after 5 seconds
    if ( AP_HAL::Calclate_dt(tnow,_state.last_time_micros) > AP_BATTMONITOR_SMBUS_TIMEOUT_MICROS) {
