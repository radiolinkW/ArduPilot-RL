...
bool AP_Arming::compass_checks(bool report)
{
    ...
    if (check_enabled(ARMING_CHECK_COMPASS)) {
        ...
        if (mag_field > AP_ARMING_COMPASS_MAGFIELD_MAX || mag_field < AP_ARMING_COMPASS_MAGFIELD_MIN) {
            check_failed(ARMING_CHECK_COMPASS, report, "005-Check mag field: %4.0f, max %d, min %d", (double)mag_field, AP_ARMING_COMPASS_MAGFIELD_MAX, AP_ARMING_COMPASS_MAGFIELD_MIN);
            return false;
        }

        // check all compasses point in roughly same direction
        if (!_compass.consistent()) {
            check_failed(ARMING_CHECK_COMPASS, report, "004-Compasses inconsistent");
            return false;
        }
        ...
        if ((magfield_error_threshold > 0) && ahrs.use_compass() && ahrs.get_location(ahrs_loc)) {
            ...
            if (MAX(fabsf(diff_mgauss.x), fabsf(diff_mgauss.y)) > magfield_error_threshold) {
                check_failed(ARMING_CHECK_COMPASS, report, "005-Check mag field (xy diff:%.0f>%d)",
            ...
            if (fabsf(diff_mgauss.x) > magfield_error_threshold*2.0) {
                check_failed(ARMING_CHECK_COMPASS, report, "005-Check mag field (z diff:%.0f>%d)",
    ...
    if (check_enabled(ARMING_CHECK_GPS)) {
        ...
        for (uint8_t i = 0; i < gps.num_sensors(); i++) {
            ...
            if (gps.status(i) < AP_GPS::GPS_OK_FIX_3D) {
                check_failed(ARMING_CHECK_GPS, report, "002-GPS %i: Bad fix", i+1);
                return false;
            }

            //GPS update rate acceptable
            if (!gps.is_healthy(i)) {
                check_failed(ARMING_CHECK_GPS, report, "001-GPS %i: not healthy", i+1);
                return false;
            }
        }

        if (!AP::ahrs().home_is_set()) {
            check_failed(ARMING_CHECK_GPS, report, "002-AHRS: waiting for home");
            return false;
        }
    ...
    }
    {
            ...
            for (const auto &channel_to_check : channels_to_check) {
                ...
                if (c->get_control_in() != 0) {
                    if ((method != Method::RUDDER) || (c != rc().get_arming_channel())) { // ignore the yaw input channel if rudder arming
                        check_failed(ARMING_CHECK_RC, true, "007-%s (RC%d) is not neutral", channel_to_check.name, c->ch());
                        check_passed = false;
                    }
                }
            }
        ...
        if (rc().arming_check_throttle()) {
            const RC_Channel *c = &rc().get_throttle_channel();
                if (c->get_control_in() != 0) {
                    check_failed(ARMING_CHECK_RC, true, "006-%s (RC%d) is not neutral", "Throttle", c->ch());
                    check_passed = false;
                }
...
bool AP_Arming::manual_transmitter_checks(bool report)
{
    if (check_enabled(ARMING_CHECK_RC)) {
        ...
        if (AP_Notify::flags.failsafe_radio) {
            check_failed(ARMING_CHECK_RC, report, "008-Radio failsafe on");
            return false;
        }
